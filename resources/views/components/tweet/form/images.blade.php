    ...
    <template x-for="(field, i) in fields" :key="i">
        <div class="w-full flex my-2">
            <label :for="field.id" class="border border-gray-300 rounded-md p-2 w-full bg-white cursor-pointer">
                <input type="file" name="images[]" :id="field.id" accept="image/*" class="sr-only" @change="fields[i].file = $event.target.files[0]">
                <span class="text-gray-700" x-text="field.file ? field.file.name : '画像を選択'"></span>
            </label>
